-- Ajusta sync_streak(): remove o check-in de saúde (fundido no Diário —
-- "diario" já cobre isso) e muda a régua de Hidratação pra 50% da meta
-- (não mais 100%). Lista de tarefas passa a ter 4 itens (Vocabulário,
-- Diário, Livros, Hidratação), continua "pelo menos 3".
