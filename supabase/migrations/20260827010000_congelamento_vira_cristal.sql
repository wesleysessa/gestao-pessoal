-- Evolui o "congelamento" (a cada 100 dias de chama acesa) pra "Cristal" 💎
-- (a cada 14 dias — 2 semanas). Mesma mecânica: acumula, e perdoa um dia
-- que falhou a meta sem apagar a chama.
alter table public.streak_estado rename column congelamentos_disponiveis to cristais_disponiveis;

-- Justo com quem já tinha uma sequência em andamento antes da troca: credita
-- os cristais que já teria ganhado sob a nova régua de 14 dias.
update public.streak_estado
set cristais_disponiveis = cristais_disponiveis + floor(streak_atual / 14)::int
where streak_atual >= 14;
