-- 1) Livros ganha updated_at, atualizado sozinho (trigger) em qualquer
--    edição — editar um livro já cadastrado passa a contar como "feito"
--    hoje, além de criar um livro novo.
alter table public.livros add column updated_at timestamptz not null default now();
...
-- 2) Chama Acesa: meta diária cai de 4 para 3 das 5 tarefas, e o critério de
-- "Livros" passa a considerar tanto um registro criado no dia quanto um
-- editado no dia.
...
begin
  ...
  while d < hoje_br loop
    ...
    if exists (
      ...
      where user_id = uid
        and (data = d or (updated_at at time zone 'America/Sao_Paulo')::date = d)
    ) then
      concluidas := concluidas + 1;
    end if;
